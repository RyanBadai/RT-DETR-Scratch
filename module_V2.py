...
# ===================================================================
# DEFINISI MODEL (Tidak ada perubahan di sini)
# ===================================================================
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        s5 = self.layer3(s4)
        s6 = self.layer4(s5)
        feat1 = self.adjust_s4(s4)
        feat2 = self.adjust_s5(s5)
...
class AIFI(nn.Module):
    ...
    def forward(self, x):
        ...
        x_reshaped = self.norm2(x_reshaped + ffn_output)
        x = x_reshaped.permute(0, 2, 1).view(B, C, H, W)
        return x

class RTDETRHybridEncoder(nn.Module):
    def __init__(self, channels=[128, 256, 512]):
        super(RTDETRHybridEncoder, self).__init__()
        self.aifi = AIFI(dim=channels[2])
        self.fusion_s5 = nn.Conv2d(channels[2] + channels[1], channels[1], kernel_size=3, padding=1)
        self.norm_s5 = nn.BatchNorm2d(channels[1])
        self.fusion_s4 = nn.Conv2d(channels[1] + channels[0], channels[0], kernel_size=3, padding=1)
        self.norm_s4 = nn.BatchNorm2d(channels[0])
        self.relu = nn.ReLU(inplace=True)
    def forward(self, features):
        s4, s5, s6 = features
        s6_enhanced = self.aifi(s6)
        s6_upsampled = F.interpolate(s6_enhanced, size=s5.shape[2:], mode='bilinear', align_corners=False)
        fused_s5 = torch.cat([s6_upsampled, s5], dim=1)
        fused_s5 = self.relu(self.norm_s5(self.fusion_s5(fused_s5)))
        s5_upsampled = F.interpolate(fused_s5, size=s4.shape[2:], mode='bilinear', align_corners=False)
        fused_s4 = torch.cat([s5_upsampled, s4], dim=1)
        fused_s4 = self.relu(self.norm_s4(self.fusion_s4(fused_s4)))
...
class RTDETRDecoder(nn.Module):
    def __init__(self, dim, num_queries=300, num_classes=3):
        ...
        self.fc_cls = nn.Linear(dim, num_classes + 1)
...
class RTDETR(nn.Module):
    def __init__(self, num_classes=3):
        ...
        self.encoder = RTDETRHybridEncoder(channels=[128, 256, 512]) 
        self.decoder = RTDETRDecoder(dim=512, num_classes=num_classes)
    def forward(self, x):
        features = self.backbone(x)
        memory_pyramid = self.encoder(features)
        memory = memory_pyramid[-1]
...
def load_model_with_pretrained_weights(
    ...
    try:
        ...
        pretrained_state_dict = load_state_dict_from_url(weights_url, map_location=device)
    ...
    for name, param in pretrained_state_dict.items():
        ...
        if clean_name in model_state_dict:
            if param.shape == model_state_dict[clean_name].shape:
                ...
                print(f"  -> Mengabaikan: {clean_name} (Ukuran tidak cocok: pre-trained {param.shape} vs kustom {model_state_dict[clean_name].shape})")
    model.load_state_dict(weights_to_load, strict=False)
    print("\n✅ Berhasil memuat bobot pre-trained.")
    print("   Kepala klasifikasi ('decoder.fc_cls') diabaikan dan akan dilatih dari awal.")
    return model

# ===================================================================
# LOSS, MATCHER, DAN TRAINING PIPELINE (DENGAN PERBAIKAN)
# ===================================================================

class WShapeIoULoss(nn.Module):
    def __init__(self, scale=4.0, d=0.0, u=0.95, r=0.5):
        super(WShapeIoULoss, self).__init__()
        self.scale = scale
        self.d = d
        self.u = u
        self.r = r
    ...
    def forward(self, pred, target):
        ...
        w1, h1 = (b1_x2 - b1_x1).clamp(min=self.eps), (b1_y2 - b1_y1).clamp(min=self.eps)
        w2, h2 = (b2_x2 - b2_x1).clamp(min=self.eps), (b2_y2 - b2_y1).clamp(min=self.eps)
        ...
        loss_wshapeiou = beta * loss_shapeiou

        return loss_wshapeiou.mean()
...
def hungarian_matcher(pred_boxes, pred_logits, target_boxes_list, target_labels_list, num_classes):
    ...
    for i in range(B):
        t_boxes = target_boxes_list[i]
        t_labels = target_labels_list[i]

        valid_gt_mask = (t_boxes[:, 2] > 0) & (t_boxes[:, 3] > 0)
        t_boxes = t_boxes[valid_gt_mask]
        t_labels = t_labels[valid_gt_mask]
        ...
        valid_labels = t_labels[t_labels < num_classes]
        if len(valid_labels) == 0:
            indices.append((torch.tensor([], dtype=torch.long), torch.tensor([], dtype=torch.long)))
            continue

        cost_class = -out_prob[:, valid_labels]
        cost_bbox = torch.cdist(pred_boxes[i], t_boxes, p=1)
        
        with torch.no_grad():
            cost_wshape = wshape_loss_fn(pred_boxes[i].unsqueeze(1), t_boxes.unsqueeze(0)).squeeze(0)
        
        C = cost_bbox + 2.0 * cost_wshape + 5.0 * cost_class
        
        # MODIFIKASI: Ganti NaN/inf dengan nan_to_num untuk keamanan ekstra
        C = torch.nan_to_num(C, nan=100000.0, posinf=100000.0, neginf=100000.0)

        C_np = C.cpu().detach().numpy()
        pred_idx, tgt_idx = linear_sum_assignment(C_np)
...
def train_one_epoch(model, optimizer, data_loader, device, num_classes):
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        for i, (pred_idx, tgt_idx) in enumerate(indices):
            ...
            current_target_boxes = target_boxes[i]
            current_target_labels = target_labels[i]

            valid_gt_mask = (current_target_boxes[:, 2] > 0) & (current_target_boxes[:, 3] > 0)
            
            if not valid_gt_mask.any():
                continue

            matched_pred_boxes = pred_boxes[i, pred_idx]
            matched_target_boxes = current_target_boxes[valid_gt_mask][tgt_idx]
            
            matched_pred_logits = pred_logits[i, pred_idx]
            matched_target_labels_one_hot = F.one_hot(
                current_target_labels[valid_gt_mask][tgt_idx].long().clamp(0, num_classes - 1), 
                num_classes=num_classes + 1
            ).float()

            loss_bbox += bbox_loss_fn(matched_pred_boxes, matched_target_boxes)
            loss_cls += focal_loss_fn(matched_pred_logits, matched_target_labels_one_hot)
            valid_batches += 1
            
        if valid_batches > 0:
            loss_bbox = loss_bbox / valid_batches
            loss_cls = loss_cls / valid_batches
            loss = 5 * loss_bbox + 2 * loss_cls
        else:
            loss = torch.tensor(0.0, requires_grad=True, device=device)
        
        if torch.isnan(loss):
            print(f"Peringatan: Loss adalah NaN di langkah {batch_idx}. Melewati update.")
            continue
    ...
    progress_bar.close()
    return total_loss / len(data_loader), time.time()

# MODIFIKASI: Menambahkan fungsi validasi yang hilang
